...
import numpy as np
# Removed 'import matplotlib.cm as cm' and will use plt.colormaps instead
from backend import (
...
def logout():
    ...
    st.session_state.page = "Home"
    st.session_state.username = None
    # st.rerun() removed here to fix the "no-op" warning
# ============================
# LOGIN/REGISTRATION PAGE FUNCTION
# ============================
def login_page():
...
if not st.session_state.logged_in:
    ...
    # --- HEADER ---
    st.markdown("""
        <h1 style='text-align:center; color:#1f77b4; margin-bottom:0;'>🕵️ Crime Data Visualization Dashboard</h1>
    """, unsafe_allow_html=True)

    # --- Create two columns: Left = Welcome message | Right = Logout button ---
    col_left, col_right = st.columns([3, 1])

    with col_left:
        st.markdown(
            f"<p style='font-size:1.2em; color:#2C3E50; text-align:left; margin-top:10px;'>👋 Welcome, <b>{st.session_state.username}</b></p>",
            unsafe_allow_html=True
        )

    with col_right:
        # Scoped CSS for ONLY logout button
        st.markdown("""
            <style>
            /* Style only the logout button */
            ...
            div[data-testid="logout-btn"] > button:hover {
                background-color: #c0392b !important;
            }
            </style>
        """, unsafe_allow_html=True)

        # Unique container with a testid
        logout_container = st.container()
        with logout_container:
            logout_container.markdown('<div data-testid="logout-btn">', unsafe_allow_html=True)
            st.button("🔓 Logout", on_click=logout, use_container_width=True, key="logout_btn")
    ...
    if st.session_state.page == "Home":
        ...
        with col5:
            # Replaced AI Chatbot with Future Prediction
    ...
    # ============================
        ...
        if state1 != state2 and state1 and state2:
            ...
            def draw_composition_pie(state, column_obj):
                ...
                # FIX: Use modern Matplotlib colormaps method
